...
export function uploadCoverController(req: Request, res: Response, next: NextFunction) {
  uploadCover(req, res, async (err: unknown) => {
    ...
    if (!req.file) {
      return next(new BadRequestError('No cover image file provided. Field name must be "cover".'));
    }
    ...
    try {
      ...
      if (!magicCheck.isValid || !magicCheck.format) {
        ...
        return next(
          new BadRequestError('File rejected: Content signature does not match valid WebP or JPEG format.')
        );
      }
